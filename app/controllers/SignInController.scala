      ...
      signInData => {
        ...
        apiAuthenticator.signin(signInData).flatMap {
          case response if response.status == OK => {
            ...
            Future.successful(Ok(views.html.secured.index(signInData.id)(id.get)).withHeaders((AUTHORIZATION, "Bearer " + tokenString)).withCookies(Cookie("evely_auth", tokenString)))
          }
